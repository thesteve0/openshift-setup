...
After that James says:
```markdown
tl;dr is to create .env with at least PULL_SECRET in it
...
if you want to add GPU nodes, create clusters/${cluster_name}.${base_domain}/values/aws-nvidia-gpu-machinesets/values.yaml and put some overrides in for that chart's values file:
